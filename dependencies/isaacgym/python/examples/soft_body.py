...
sim = gym.create_sim(
    args.compute_device_id, args.compute_device_id, args.physics_engine, sim_params
)
if sim is None:
...
soft_thickness = (
    0.1  # important to add some thickness to the soft body to avoid interpenetrations
)
...
for i in range(asset_soft_body_count):
    ...
    print(
        f"(Body {i}) youngs: {mat.youngs} poissons: {mat.poissons} damping: {mat.damping}"
    )
...
for i in range(num_envs):
    ...
    for j in range(asset_soft_body_count):
        ...
        actor_soft_materials[j].poissons = random.uniform(
            poissons * 0.8, poissons * 1.2
        )
...
while not gym.query_viewer_has_closed(viewer):
    ...
    if flag_draw_contacts:
        ...
        for env in envs:
            gym.draw_env_soft_contacts(
                viewer, env, gymapi.Vec3(0.7, 0.2, 0.15), 0.0005, True, True
            )
    ...
    if flag_compute_pressure:
        ...
        for env in envs:
            ...
            for i in range(tri_range.start, tri_range.start + tri_range.count):
                ...
                stress = np.matrix(
                    [
                        (parent_stress.x.x, parent_stress.y.x, parent_stress.z.x),
                ...
                normal = np.array(
                    (tri_normals[i].x, tri_normals[i].y, tri_normals[i].z)
                )
